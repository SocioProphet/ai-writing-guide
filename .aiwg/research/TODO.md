...
### GAP-007: Optimal Code Embedding Span Size

- **Claim**: "Function-body spans are the optimal granularity for code embedding retrieval"
- **Current Evidence**: GRADE: LOW — Moatless uses function-body spans and achieves 70.8% SWE-bench, but no controlled comparison to other granularities
- **Needed**: Ablation study comparing file-level, class-level, function-level, and line-chunk embedding granularity on retrieval precision/recall
- **Search Terms**: code embedding granularity, code retrieval span size, embedding unit code search accuracy
- **Filed By**: agent-codebase-navigation-research
- **Filed Date**: 2026-03-01
- **Status**: Open
- **Priority**: High — directly affects AIWG index design

### GAP-008: Code Embedding Model Selection for TypeScript/JavaScript

- **Claim**: "Voyage voyage-code-3 is the best embedding model for code retrieval"
- **Current Evidence**: GRADE: LOW — Moatless uses it for SWE-bench (Python-heavy), no evidence for TypeScript/JavaScript specifically
- **Needed**: Comparative benchmark of code embedding models (voyage-code-3, OpenAI text-embedding-3, Cohere embed-code) on TypeScript/JavaScript retrieval tasks
- **Search Terms**: code embedding model benchmark TypeScript, JavaScript code search embedding comparison
- **Filed By**: agent-codebase-navigation-research
- **Filed Date**: 2026-03-01
- **Status**: Open
- **Priority**: High — AIWG is primarily TypeScript

### GAP-009: Mini-SWE-agent Navigation Strategy

- **Claim**: "Minimal shell-tool navigation can match full-index approaches in performance"
- **Current Evidence**: GRADE: VERY LOW — mini-SWE-agent claims 65% SWE-bench verified in 100 lines, but strategy details not yet published
- **Needed**: Source inspection of mini-SWE-agent or published technical report on its navigation approach
- **Search Terms**: mini-SWE-agent implementation, minimal agent code navigation, SWE-bench lightweight agent
- **Filed By**: agent-codebase-navigation-research
- **Filed Date**: 2026-03-01
- **Status**: Open
- **Priority**: Medium — may change design recommendations

### GAP-010: Monorepo Multi-Language Index Strategies

- **Claim**: "The three-index approach (symbol graph + inverted + vector) works for multi-language monorepos"
- **Current Evidence**: GRADE: VERY LOW — all inspected systems are tested primarily on Python repos (SWE-bench)
- **Needed**: Evidence from production systems handling TypeScript/JavaScript/Python mixed-language repos
- **Search Terms**: multi-language code index monorepo, polyglot repository navigation agent, mixed language codebase search
- **Filed By**: agent-codebase-navigation-research
- **Filed Date**: 2026-03-01
- **Status**: Open
- **Priority**: High — AIWG repos are typically multi-language
